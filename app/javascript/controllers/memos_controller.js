...
export default class extends Controller {
  ...
  connect() {
    if (
      document.getElementsByClassName("as-waveform")[0].dataset.firstLoad ==
      "true"
    ) {
      this.setWaveforms();
    }
  ...
  setWaveforms() {
    ...
    currentSample = ``;
    previousSample = `${
      document.getElementsByClassName("as-waveform")[0].dataset.sample_name
    }`;
    this.oldSampleName = `${
      document.getElementsByClassName("as-waveform")[0].dataset.sample_name
    }`;
  ...
  playOrPauseAudio(event) {
    ...
    if (
      ...
      (previousSample != currentSample &&
        playPauseStatObj[previousSample] == 1) ||
      (previousSample == currentSample && playPauseStatObj[currentSample] == 0)
    ) {
      if (
        previousSample != currentSample &&
        playPauseStatObj[previousSample] == 1
        ...
        samplePlayTriangle = document.getElementById(
          "audio".concat(this.oldSampleName, "_tri")
        );
        samplePauseBars = document.getElementById(
          "audio".concat(this.oldSampleName, "_bar")
        );
      ...
      //Play/Pause Icon switch
      samplePlayTriangle = document.getElementById(
        "audio".concat(sampleName, "_tri")
      );
      samplePauseBars = document.getElementById(
        "audio".concat(sampleName, "_bar")
      );
  ...
  bookmarkMemo(e) {
    document
      .querySelector(`#bookmark-memo-${e.target.dataset.memoId}`)
      .classList.toggle("unbookmarked");
    document
      .querySelector(`#bookmark-memo-${e.target.dataset.memoId}`)
