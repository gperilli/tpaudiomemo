...
export default class extends Controller {
  ...
  connect() {
    if (
      document.getElementsByClassName("new-as-waveform")[0].dataset.firstLoad ==
      "true"
    ) {
      this.setWaveforms();
    }
  ...
  setWaveforms() {
    const totalSamplesN =
      document.getElementsByClassName("new-as-waveform").length;
    ...
    currentSample = ``;
    previousSample = `${
      document.getElementsByClassName("new-as-waveform")[0].dataset.sample_name
    }`;
  }
  ...
  showNewMemoModal(e) {
    ...
    document.querySelector("#memo-submit-button").dataset.memoId =
      e.target.dataset.memoId;

    const updateOrNew = e.target.dataset.updateOrNew;

    document.querySelector("#update-or-new-modal-title").innerHTML =
      updateOrNew == "new" ? "Take Memo" : "Update Memo";

    if (updateOrNew == "update") {
      document.querySelector("#memo-form-name-input").value =
        document.querySelector(
          `#memo-name-${e.target.dataset.memoId}`
        ).innerHTML;
      document.querySelector("#memo-form-notes-input").value =
        document.querySelector(
          `#memo-notes-${e.target.dataset.memoId}`
        ).innerHTML;
    } else {
      document.querySelector("#memo-form-name-input").value = "";
      document.querySelector("#memo-form-notes-input").value = "";
    ...
    if (updateOrNew == "update") {
      ...
      document.querySelectorAll(
        ".as-play-button__svg"
      )[1].dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/${memoUpdateOrSaveCurrentAudioKey}`;
      document.querySelectorAll(
        ".as-play-button__svg"
      )[1].dataset.audiofilekey = memoUpdateOrSaveCurrentAudioKey;

      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarOne"
      ).dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/${memoUpdateOrSaveCurrentAudioKey}`;
      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarTwo"
      ).dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/${memoUpdateOrSaveCurrentAudioKey}`;
      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarOne"
      ).dataset.audiofilekey = memoUpdateOrSaveCurrentAudioKey;
      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarTwo"
      ).dataset.audiofilekey = memoUpdateOrSaveCurrentAudioKey;

      document.querySelector(
        "#audiovoicemailrecording_tri"
      ).dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/${memoUpdateOrSaveCurrentAudioKey}`;
      document.querySelector(
        "#audiovoicemailrecording_tri"
      ).dataset.audiofilekey = memoUpdateOrSaveCurrentAudioKey;
      ...
      document.querySelector("#audio-player").dataset.audiofilekey =
        memoUpdateOrSaveCurrentAudioKey;

      document.querySelector(
        "#initial-recording-output-overlay"
      ).style.display = "none";
    } else {
      ...
      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarOne"
      ).dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/kkx6uxfz2qv9ji1oktjg5f0i61hm`;
      document.querySelector(
        "#newOrUpdatePlayPauseButoonBarTwo"
      ).dataset.audiosource = `https://res.cloudinary.com/gperilli/video/upload/v1/development/kkx6uxfz2qv9ji1oktjg5f0i61hm`;
      document.querySelector(
      ...
      document.querySelector(
        "#audiovoicemailrecording_tri"
      ).dataset.audiofilekey = `kkx6uxfz2qv9ji1oktjg5f0i61hm`;
